...
3. **🎬 動畫**：`.player` 內一張 inline SVG（viewBox 約 `0 0 920 430`），6–10 個分鏡
   ...
   - 情境取自〈範例情境：風電運維〉，例如技師輸入「葉片旋角馬達故障」查運維手冊
4. **🧩 重點整理**：`.keypoints` 4 張卡片
5. **🧪 互動範例**：瀏覽器內真的會運算的小 demo（善用 `RagTutor.rt`：`tokenize`、`buildIndex`、`searchVector`、`searchBM25`、`rrf`、`chunk`、`highlight`），讓使用者可調參數、比較「用 / 不用這個技術」的差異。語料用〈範例情境：風電運維〉的虛構《OW-8 運維手冊》內容（繁體中文），每堂換不同的子系統章節或輔助文件。
...
所有課程的動畫、互動範例與程式範例都以**風機運維手冊的檢索**為主軸：現場技師用 RAG 查手冊，例如輸入「葉片旋角馬達故障」，系統找出相關的故障原因、排除步驟與安全注意事項。

- **場景設定（虛構）**：「鷗灣離岸風場」，風機編號 `WTG-01`～`WTG-40`，機型「OW-8」；需要新舊機型對照時用舊款「OW-6」，需要陸域對照時用「山嵐陸域風場」。
- **主要知識庫：《OW-8 運維手冊》**，依子系統分章：葉片、葉片旋角系統（pitch）、偏航系統（yaw）、主軸承、齒輪箱、發電機、變流器、液壓與煞車、塔架與基礎。每節包含告警代碼、可能原因、排除步驟、所需工具與備品、安全注意事項。
- **輔助文件**（主題需要時才用）：維修工單、巡檢報告（葉片無人機巡檢、塔架目視）、狀態監測系統（CMS）振動與油液分析報告、技術通報、備品料號表、工安規範（高處作業、登船、上鎖掛牌 LOTO）、天候與海象紀錄。
- **提問的人**：現場技師、運維工程師、遠端監控中心值班人員、備品管理人員。
- **用語**：依台灣用法寫「旋角」（pitch），不寫「變槳」；另有「偏航」（yaw）、「順槳」（feathering）。
- **常用告警代碼（虛構，跨課程固定意義）**：`PT-203` 旋角馬達過溫、`PT-205` 旋角馬達煞車異常、`PT-211` 三支葉片角度偏差過大、`PT-230` 旋角備援電源測試失敗、`YW-042` 偏航誤差過大、`GB-117` 齒輪箱高速軸軸承溫度過高、`CV-310` 變流器功率模組過溫。新代碼沿用 `子系統縮寫-三位數`；工單如 `WO-2026-0412`，料號如 `PT-MTR-02`。
- **正確性**：
  - 元件功能、常見故障模式、維護方式等技術敘述要查證（IEC 61400 系列、原廠公開文件、學術論文），不確定就不寫。
  - 告警代碼、工單、料號、門檻值、日期、人名一律虛構；頁面用 `.callout.warn` 註明「風場、機型、手冊內容與數據皆為虛構，實際作業以原廠手冊與現場規範為準」。
  - 不使用真實風場名稱，也不冒用真實廠牌的機型或告警代碼。
- `lessons/roadmap.json` 每個主題的 `scenario` 是建議情境，可依教學需要調整。
- 第 001–004 堂保留原本的語料，互動範例另外加上「風機運維手冊」語料切換；之後的新課程直接用風電運維語料。
